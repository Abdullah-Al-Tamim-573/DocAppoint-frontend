...
import TopSpecialistsCard from "../TopSpecialistsCard/TopSpecialistsCard";
 ...
 let TopSpecialists = async() => {
    ...
    let filterHighRatedDoctors = doctorsData.filter(highRatedDoctors => 4.8 < highRatedDoctors.rating);
  ...
  return (
    <div className="max-w-[85%] mx-auto px-4 py-12">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button className="hidden sm:flex items-center gap-2 border border-teal-500 text-teal-600 px-5 py-2 rounded-full hover:bg-teal-50 transition-all duration-300">
          View All Doctors
          <ArrowRight size={16} />
        </button>
      </div>
      ...
      {/* Doctors Card */}
      <div className={`${doctorsData.length === 0? 'my-10 xl:my-20' : 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6'}`}>
          {
              doctorsData.length === 0 ? 
              <h3 className="text-2xl xl:text-4xl text-red-600  flex justify-center ">Doctors Data Not Found</h3>
              : 
             filterHighRatedDoctors.map(doctor => <TopSpecialistsCard key={doctor._id} doctor={doctor}></TopSpecialistsCard> )
          }
      </div>
